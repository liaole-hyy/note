...
1.在开发测试环境中，我们一般搭建Redis的单实例来应对开发测试需求，但是在生产环境，如果对可用性、可靠性要求较高，则需要引入Redis的集群方案。
...
![image-20210529171207899](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210529171207899.png)
...
logfile /usr/local/redis/logs/redis.log # 日志文件的位置,不更改也可以 
...
```
replicaof 192.168.5.128 6379 # master的ip，port
masterauth 123456 # master的密码
replica-serve-stale-data no # 如果slave无法与master同步，设置成slave不可读，方便监控脚本发现问题
```

本示例在单台服务器上配置master端口6379，两个slave端口分别为6379,6379，启动master，再启动两个slave

```
各自机器执行
[root@dev-server-1 master-slave]# redis-server redis.conf
[root@dev-server-1 master-slave]# redis-server redis.conf
[root@dev-server-1 master-slave]# redis-server redis.conf
```
...
[root@dev-server-1 master-slave]# redis-cli -p 6379
127.0.0.1:6379> auth 123456
OK
127.0.0.1:6379> get site
"blog.jboost.cn"
...
![image-20210529171942667](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210529171942667.png)
...
![image-20210529173232984](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210529173232984.png)
...
![image-20210529173655877](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210529173655877.png)
